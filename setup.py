...
The version is read from src/ebus_mdns_discovery/__init__.py's __version__ (the
...
tables; without this the `mdns-discovery` command would not be installed in a
...
version = re.search(
    r'^__version__ = "([^"]+)"',
    Path("src/ebus_mdns_discovery/__init__.py").read_text(encoding="utf-8"),
    re.M,
).group(1)

setup(
    name="ebus-mdns-discovery",
    version=version,
    package_dir={"": "src"},
    packages=["ebus_mdns_discovery"],
    package_data={"ebus_mdns_discovery": ["py.typed"]},
    entry_points={
        "console_scripts": [
            "mdns-discovery = ebus_mdns_discovery.service:run",
        ],
    },
)
